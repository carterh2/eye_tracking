...
```
# Based off of these results we should rerun a model with age_gender_sat_bright
```{r}
model_final <- nlme(
  log_duration ~ a - b * Order_log,
...
```
This did not improve model metrics to eliminate ROI categories
```{r AIC better}

anova(model_roi_cat, model_sat_bright)
```	
